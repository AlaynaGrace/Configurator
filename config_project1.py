...
def config_project():
        ...
        infile = '/users/jbuysse/desktop/config.xlsx'
        ##window = tk.Tk()
        ##window.resizable(0,0)
        ##f = ttk.Frame(window).grid(column = 0, row = 0)
        ##ttk.Label(f, text = 'Path name: ').grid(column = 0, row = 0)
        ##
        ##v = tk.StringVar()
        ...
        ##    
        ##
        ##submit = ttk.Button(f, text='Submit', command=sub)
        ##
        ...
        df = pd.read_excel(infile, sheetname = 0, parse_cols="A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,P,Q,R",names=('application',
        ...
        def submit_button():
                ...
                #varlist[5] = float(varlist[5])

                

                dfindex = df.set_index(['application', 'trans_mfg','trans_model','trans_opening','pto_mfg','gear_ratio','shift_option'])

                ptoCode = dfindex.ix[output[0],output[1],output[2],output[3],output[4],

                output[5],output[6]]

                code = ptoCode['mfg']
                pumpRotation = dfindex.ix[output[0],output[1],output[2],output[3],output[4],output[5],output[6]]
                a = ttk.Label(frame1, text = "PTO: " + str(code))
                pumpRotation = pumpRotation['pump_rotation']

                a.grid(column = 0, row = 8)
        ...
        def qvar5_callback(*args):
                ...
                if qvar5.get():
                        ...
                        index =df.set_index(['application', 'trans_mfg','trans_model','trans_opening','pto_mfg','gear_ratio'])

                        index = index.ix[varlist[0].get(),varlist[1].get(),varlist[2].get(),varlist[3].get(),varlist[4].get(), varlist[5].get()]

                        index = index['shift_option']
        ...
        def acc_submit():
            ...
            ttk.Label(frame5, text = 'Part Numbers: ' + str(partList)).grid(column = 0, row = 8)
